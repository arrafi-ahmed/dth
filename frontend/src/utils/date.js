...
/**
 * Format a UTC date string into a localized string using the saved user timezone
 * @param {string|object} input - The UTC ISO date string or an object with input property
 * @param {object} options - Intl.DateTimeFormat options
 * @returns {string} Formatted localized date/time string
 */
export function formatDateTime(input, options = {}) {
  // Handle polymorphic input (string or { input: '...' })
  const dateStr = typeof input === 'object' && input?.input ? input.input : input
  if (!dateStr) return ''

  const user = store?.state?.auth?.currentUser
  const timezone = user?.timezone || 'UTC'

  // Intl.DateTimeFormat throws if styles (dateStyle/timeStyle) are mixed with components (month/day/etc)
  const hasStyle = options.dateStyle || options.timeStyle

  const baseOptions = hasStyle ? { timeZone: timezone } : {
    month: 'short',
    day: 'numeric',
    year: 'numeric',
    hour: '2-digit',
  ...
  try {
    return new Intl.DateTimeFormat('en-US', { ...baseOptions, ...options }).format(new Date(dateStr))
  } catch (e) {
