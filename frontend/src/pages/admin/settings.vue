<script setup>
  ...
  const { variant, density, size } = useUiProps()
  ...
  async function saveBanner () {
    ...
    const sortedDates = formData.value.dateRange.toSorted((a, b) => {
  ...
  async function fetchFooterSettings () {
    try {
      ...
      await store.dispatch('settings/fetchFooter')
      const settings = store.state.settings.footer
      footerSettings.value = { ...settings }
    } catch (error) {
  ...
  async function saveFooterSettings () {
    try {
      footerLoading.value = true
      await store.dispatch('settings/updateFooter', footerSettings.value)
    } catch (error) {
  ...
  async function fetchHeaderSettings () {
    try {
      ...
      await store.dispatch('settings/fetchHeader')
      const settings = store.state.settings.header
      headerSettings.value = { ...settings }
  ...
  async function saveHeaderSettings () {
    try {
      headerLoading.value = true
      const data = new FormData()
      if (logoFile.value instanceof File) data.append('logoImage', logoFile.value)
      if (logoFileDark.value instanceof File) data.append('logoImageDark', logoFileDark.value)
      ...
      await store.dispatch('settings/updateHeader', data)
  ...
  async function fetchAppearanceSettings () {
    try {
      ...
      await store.dispatch('settings/fetchAppearance')
      const settings = store.state.settings.appearance
      appearanceSettings.value = { ...settings }
      advancedJson.value = {
        light: JSON.stringify({ colors: settings.lightColors, variables: settings.lightVariables }, null, 2),
        dark: JSON.stringify({ colors: settings.darkColors, variables: settings.darkVariables }, null, 2),
      }
    } catch (error) {
  ...
  async function saveAppearanceSettings () {
    try {
      appearanceLoading.value = true
      await store.dispatch('settings/updateAppearance', appearanceSettings.value)
